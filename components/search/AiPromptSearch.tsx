...
export default function Search({
  ...
  const askSigPilotRef = useRef<HTMLButtonElement>(null);
  ...
  // Keyboard shortcut: Cmd/Ctrl + I to focus search input
  useEffect(() => {
  ...
  const handleSearch = useDebouncedCallback(async (term) => {
    ...
    if (term) {
      ...
      try {
        ...
        setMatchingTags(Array.from(uniqueTags));
  ...
  const handleAskSigPilot = () => {
    if (searchTerm.trim()) {
      openWithPrompt(searchTerm.trim(), true, true); // autoSubmit = true
      handleResultSelection();
    }
  };

  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    // Escape should always work to blur and close
    if (e.key === 'Escape') {
      e.preventDefault();
      setOpen(false);
      setSelectedIndex(-1);
    ...
    // Check if Ask SigPilot option is available
    const hasSigPilotOption = !!searchTerm;

    if (e.key === 'ArrowDown') {
      e.preventDefault();
      if (hasSigPilotOption && selectedIndex === -1) {
        setSelectedIndex(0); // Select Ask SigPilot
      }
    ...
    } else if (e.key === 'Enter' && selectedIndex === 0 && hasSigPilotOption) {
      e.preventDefault();
      handleAskSigPilot();
    }
  };
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Immediately filter vendors for instant response
    if (term) {
  ...
  return (
    <div className="relative flex max-w-6xl justify-center">
      ...
      <div className="relative">
        <Input
          ref={inputRef}
          className={`${size === 'sm' ? 'h-8' : 'h-10'} w-[36rem] rounded-sm bg-transparent text-sm placeholder:opacity-70 hover:bg-hover focus:bg-background/90 focus-visible:ring-1 dark:bg-transparent`}
      ...
      {open && (
        ...
        >
          <div className="max-h-96 overflow-y-auto p-2 3xl:max-h-[600px]">
            {/* Ask Lineage AI Section - shows when there's a search term */}
            {searchTerm && (
              <div className="mb-2 border-b pb-2">
                <button
                  ref={askSigPilotRef}
                  onClick={handleAskSigPilot}
                  className={`flex w-full items-center gap-3 rounded-sm p-2 text-left hover:bg-hover ${selectedIndex === 0 ? 'bg-hover' : ''}`}
                >
                  <div className="flex h-8 w-8 flex-shrink-0 items-center justify-center overflow-clip rounded-sm">
                    <img
                      src="/sigpilot_1.png"
                      alt="Lineage AI Assistant"
                      width={24}
                      height={24}
                  ...
                  <div className="flex-1 overflow-hidden">
                    <div className="font-medium text-sm">
                      Ask LineageAI<sup>TM</sup> Assistant
                      {selectedIndex === 0 && (
            ...
            (matchingTags.length > 0 ||
                  ...
                  <>
                    ...
                    <ul className="pb-4">
                      {matchingTags.map((tag) => (
                        <li key={tag}>
                          <Link
                            href={`/contracts?tags=${tag.toLowerCase()}`}
              ...
              <ul>
                <li className="p-2 text-xs text-muted-foreground">Vendors</li>
                {vendors.map((vendor) => (
                  <li
                    key={vendor.id}
                    className="border-b border-border/70 last:border-b-0"
