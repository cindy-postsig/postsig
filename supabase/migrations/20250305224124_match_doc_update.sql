...
alter table "public"."documents" add column "contract_id" integer;

CREATE INDEX documents_contract_id_idx ON public.documents USING btree (contract_id);

alter table "public"."documents" add constraint "documents_contract_id_fkey" FOREIGN KEY (contract_id) REFERENCES contracts(id) ON UPDATE CASCADE ON DELETE CASCADE not valid;

alter table "public"."documents" validate constraint "documents_contract_id_fkey";
...
BEGIN   
   ...
   AND (documents.contract_id = contract_id OR contract_id IS NULL) 
