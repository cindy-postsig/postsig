-- Make admin_activity_logs fast under PostgREST's parameterized prepared
-- statements.
--
-- Background: 20260512120400_app_visit_activity.sql declared the function
-- `security definer set search_path = ''`. Both clauses disable SQL function
-- inlining, so the body executed as an opaque SRF and the planner produced
-- a poor plan (~10s in dev).
--
-- Removing those clauses alone is not enough: when PostgREST calls the
-- function with bound parameters (all NULL for the unfiltered admin logs
-- page), the planner cannot fold the `(p_x IS NULL OR e.col = p_x)` branches
-- and gets terrible row estimates. It pushes those branches down into
-- `app_visits` and chooses a Nested Loop on `auth.audit_log_entries × users`
-- with ~5M comparisons. Direct calls with literal NULL are fine (Hash Join);
-- the parameterized form blows up.
--
-- This migration:
--   1. Drops the stale 9-arg overload left behind by
--      20260512000000_admin_activity_logs.sql (replaced by the 10-arg
--      variant in 20260512120400 — `create or replace` does not delete
--      prior signatures).
--   2. Recreates the 10-arg function without `security definer` and without
--      `set search_path`. Access remains restricted by the existing
--      `grant execute ... to service_role` (and revokes for public /
--      authenticated). All table references in the body are schema-qualified.
--   3. Marks the `app_visits` CTE `MATERIALIZED` so the planner cannot push
--      outer predicates into the `auth.audit_log_entries` scan. The CTE is
--      computed once with accurate row estimates and a Hash Join, which
--      keeps the function under ~400ms regardless of parameter binding.
--   4. Grants `service_role` SELECT on `auth.audit_log_entries` (needed now
--      that the function runs as the caller instead of the function owner).
...
-- Without `security definer`, the function runs as the caller. service_role
-- needs explicit SELECT on auth.audit_log_entries to populate the APP_VISIT
-- source. service_role is the privileged server-side role and already
-- bypasses RLS; this grant only extends it to this one auth-schema table.
grant select on auth.audit_log_entries to service_role;
