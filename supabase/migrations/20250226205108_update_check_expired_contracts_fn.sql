...
BEGIN
    ...
    WITH updated_contracts AS (
        ...
        WHERE c.id = ctu.id
        AND (c.renewal_type != 'Auto' || c.renewal_type IS NOT NULL)
        RETURNING
            c.id,
            'unconfirmed'::contract_status as new_status,
            (c.term_end_date->0->>'date')::date as end_date
    )
    INSERT INTO contracts_to_update (id, old_status, end_date, renewal_type)
    SELECT
        uc.id,
        ctu.old_status,
        uc.end_date,
    ...
    SELECT
        ...
        END as new_status,
        CASE
            WHEN ar.id IS NOT NULL THEN ar.end_date
            ELSE ctu.end_date
        END as end_date
    FROM contracts_to_update ctu
