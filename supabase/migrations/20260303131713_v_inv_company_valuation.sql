...
drop view if exists "public"."v_inv_fund_summary";

drop view if exists "public"."v_inv_position";
...
create or replace view "public"."v_inv_position" as  SELECT t.organization_id,
    ...
    COALESCE(sum(t.amount) FILTER (WHERE (t.transaction_type = ANY (ARRAY['sale'::text, 'secondary_sale'::text, 'redemption'::text, 'exit'::text, 'distribution'::text, 'dividend'::text]))), (0)::numeric) AS realized_proceeds,
        CASE
...
create or replace view "public"."v_inv_company_valuation" as  SELECT ic.id AS company_id,
   ...
   FROM (((((((((public.inv_company ic
     ...
     LEFT JOIN LATERAL ( WITH fund_ids AS (
                 SELECT f_1.id,
                        min(t.transaction_date) FILTER (WHERE t.transaction_type = 'purchase'::text) AS first_purchase_date
                   FROM (public.inv_transaction t
...
create or replace view "public"."v_inv_fund_summary" as  SELECT p.organization_id,
    p.fund_id,
    p.fund_name,
