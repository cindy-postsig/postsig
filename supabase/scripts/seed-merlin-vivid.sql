...
SET local app.org_id = '2896b100-72b0-4a93-92bf-f49574086cad';  -- merlin
SET local app.company_id = '84';
...
WITH new_round AS (
  INSERT INTO public.inv_financing_round (
    ...
    'ext-series-a',                              -- external_id (text, optional, unique per org)
    'Series A Stock Financing',                  -- name (text, required)
    4,                                           -- stage_id (bigint, optional, FK -> inv_stages)
    '2026-02-26',                                -- announced_date (date, optional)
    '2026-02-26',                                -- initial_close_date (date, optional)
    '2026-02-26',                                -- final_close_date (date, optional, must be >= initial_close_date)
    0,                                    -- pre_money_valuation (numeric(20,2), optional)
    'USD',                                       -- currency (char(3), default 'USD')
    NULL,                                        -- notes (text, optional)
    '{}',                                        -- metadata (jsonb, default '{}')
    29370922                                      -- total_raised (numeric(20,2), optional)
  )
  RETURNING id
),
round_terms AS (
  INSERT INTO public.inv_round_terms (
    ...
    '2026-02-26',                                -- effective_date (date, required)
...
cap_snapshot AS (
  INSERT INTO public.inv_cap_table_snapshot (
    ...
    'ext-series-a',                                -- external_id (text, optional)
    '2026-02-26',                                  -- snapshot_date (date, required, unique per company+type)
    1,                                             -- snapshot_type_id (bigint, optional, FK -> inv_snapshot_types)
    15004554,                                          -- common_authorized (bigint, optional)
    5012853,                                          -- common_outstanding (bigint, optional)
    4366072,                                          -- preferred_authorized (bigint, optional)
    8136887,                                          -- preferred_outstanding (bigint, optional)
    23141441,                                          -- total_outstanding (bigint, optional)
    23141441,                                             -- fully_diluted_total (bigint, required)
    0,                                          -- option_pool_authorized (bigint, optional)
    0,                                       -- option_pool_outstanding (bigint, optional)
    0,                                          -- option_pool_available (bigint, optional)
    ...
    NULL,                                        -- our_fd_ownership_percent (numeric(7,6), optional)
    NULL,                                        -- our_preferred_pct (numeric(7,6), optional)
    NULL,                                          -- our_voting_pct (numeric(7,6), optional)
    8.6008,                                   -- share_price (numeric(20,8), optional)
    125000000,                                      -- implied_valuation (numeric(20,2), optional)
    '{}',                                          -- cap_table_detail (jsonb, default '{}')
    508425,                                          -- conversion_shares_issued (bigint, optional)
    3414903,                                          -- new_money_shares_issued (bigint, optional)
...
co_investor AS (
  INSERT INTO public.inv_co_investor (
    ...
    (SELECT id FROM new_round),                  -- financing_round_id (bigint, required, FK -> inv_financing_round)
    NULL,                                        -- external_id (text, optional)
    'Team8 Capital III, L.P.',                          -- investor_name (text, required, unique per round)
  ...
  (
    ...
    (SELECT id FROM new_round),
    NULL,
    'Glilot Early Growth Fund II L.P.',
    'Fund',
    'participant',
    false,
    NULL,
  ...
  (
    ...
    (SELECT id FROM new_round),
    NULL,
    'XPS.VC APR 2025, A SERIES OF CGF2021 LLC',
    'Fund',
    'participant',
    false,
    NULL,
...
WITH new_security AS (
  INSERT INTO public.inv_security (
    ...
    'Series A Preferred Stock',                        -- name (text, required, unique per company)
    'preferred',                                 -- security_type (text, required: common|preferred|safe|convertible_note|warrant|option)
    'Series A',                                        -- series_name (text, optional)
...
INSERT INTO public.inv_security_terms (
  ...
  '2026-02-26',                                -- effective_date (date, required)
  NULL,                                        -- superseded_date (date, optional, must be > effective_date)
  8.6008,                                      -- original_issue_price (numeric(20,8), optional)
  NULL,                                        -- conversion_price (numeric(20,8), optional)
  1.0,                                         -- conversion_ratio (numeric(20,10), default 1.0)
  0.000001,                                    -- par_value (numeric(20,10), optional)
  3841164,                                     -- authorized_shares (bigint, optional)
...
-- Chain 2b: security -> security_terms (Series A-1 Preferred Stock)
WITH new_security AS (
  INSERT INTO public.inv_security (
    organization_id,
    company_id,
    ...
    current_setting('app.company_id')::bigint,   -- company_id (bigint, required, FK -> inv_company)
    NULL,                                        -- external_id (text, optional, unique per org)
    'Series A-1 Preferred Stock',                -- name (text, required, unique per company)
    'preferred',                                 -- security_type (text, required: common|preferred|safe|convertible_note|warrant|option)
    'Series A-1',                                -- series_name (text, optional)
...
INSERT INTO public.inv_security_terms (
  ...
  '2026-02-26',                                -- effective_date (date, required)
  NULL,                                        -- superseded_date (date, optional, must be > effective_date)
  7.3107,                                      -- original_issue_price (numeric(20,8), optional)
  NULL,                                        -- conversion_price (numeric(20,8), optional)
  1.0,                                         -- conversion_ratio (numeric(20,10), default 1.0)
  0.000001,                                    -- par_value (numeric(20,10), optional)
  524908,                                      -- authorized_shares (bigint, optional)
  NULL,                                        -- issued_shares (bigint, optional)
  NULL,                                        -- outstanding_shares (bigint, optional)
  1.0,                                         -- liquidation_multiplier (numeric(5,2), default 1.0)
  NULL,                                        -- liquidation_seniority (smallint, optional)
...
-- Chain 2c: security -> security_terms (Series Seed Preferred Stock)
WITH new_security AS (
  INSERT INTO public.inv_security (
    organization_id,
    company_id,
    ...
    'preferred',                                 -- security_type (text, required: common|preferred|safe|convertible_note|warrant|option)
    'Series Seed',                               -- series_name (text, optional)
    false,                                       -- is_valuation_reference (boolean, default false)
    '{}'                                         -- metadata (jsonb, default '{}')
  )
  RETURNING id
...
INSERT INTO public.inv_security_terms (
  ...
  1.0,                                         -- conversion_ratio (numeric(20,10), default 1.0)
  0.000001,                                    -- par_value (numeric(20,10), optional)
  3770815,                                     -- authorized_shares (bigint, optional)
  NULL,                                        -- issued_shares (bigint, optional)
  NULL,                                        -- outstanding_shares (bigint, optional)
  1.0,                                         -- liquidation_multiplier (numeric(5,2), default 1.0)
  NULL,                                        -- liquidation_seniority (smallint, optional)
...
WITH new_equity_plan AS (
  INSERT INTO public.inv_equity_plan (
    ...
    '2026-02-26',                                -- adoption_date (date, optional)
    NULL,                                        -- expiration_date (date, optional)
    '{}'                                         -- metadata (jsonb, default '{}')
  )
  RETURNING id
...
INSERT INTO public.inv_equity_plan_snapshot (
  ...
  NULL,                                        -- external_id (text, optional)
  '2026-02-26',                                -- effective_date (date, required, unique per plan)
  2500000,                                     -- authorized_shares (bigint, optional)
  0,                                     -- issued_shares (bigint, optional)
  2500000,                                     -- outstanding_options (bigint, optional)
