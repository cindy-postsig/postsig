...
SET local app.org_id = '867d5688-8a32-4c7d-9bad-88d1352d2826';  -- differential
SET local app.company_id = '33';
...
WITH new_round AS (
  INSERT INTO public.inv_financing_round (
    ...
    'ext-series-b',                              -- external_id (text, optional, unique per org)
    'Series B Stock Financing',                  -- name (text, required)
    5,                                           -- stage_id (bigint, optional, FK -> inv_stages)
    '2026-01-13',                                -- announced_date (date, optional)
    '2026-01-13',                                -- initial_close_date (date, optional)
    '2026-01-13',                                -- final_close_date (date, optional, must be >= initial_close_date)
    24504428,                                    -- pre_money_valuation (numeric(20,2), optional)
    'USD',                                       -- currency (char(3), default 'USD')
    NULL,                                        -- notes (text, optional)
    '{}',                                        -- metadata (jsonb, default '{}')
    3255250                                      -- total_raised (numeric(20,2), optional)
  )
  RETURNING id
),
round_terms AS (
  INSERT INTO public.inv_round_terms (
    ...
    '2026-01-13',                                -- effective_date (date, required)
    NULL,                                        -- superseded_date (date, optional, must be > effective_date)
    NULL,                                        -- major_investor_threshold_amount (numeric(20,2), optional)
    894826,                                      -- major_investor_threshold_shares (bigint, optional)
    NULL,                                        -- major_investor_threshold_ownership_pct (numeric(7,6), optional)
    ...
    NULL,                                        -- rofr_cosale (boolean, optional)
    NULL,                                        -- investors_subject_to_rofr (boolean, optional)
    ...
    false,                                       -- required_closing_payments (boolean, default false)
    25000,                                       -- investor_counsel_fee_cap (numeric(20,2), optional)
    true,                                        -- issuer_pays_investor_counsel (boolean, optional)
    NULL,                                        -- qsbs_covenant_given (boolean, optional)
...
cap_snapshot AS (
  INSERT INTO public.inv_cap_table_snapshot (
    ...
    'ext-series-b',                                -- external_id (text, optional)
    '2026-01-13',                                  -- snapshot_date (date, required, unique per company+type)
    1,                                             -- snapshot_type_id (bigint, optional, FK -> inv_snapshot_types)
    NULL,                                          -- common_authorized (bigint, optional)
    NULL,                                          -- common_outstanding (bigint, optional)
    NULL,                                          -- preferred_authorized (bigint, optional)
    NULL,                                          -- preferred_outstanding (bigint, optional)
    NULL,                                          -- total_outstanding (bigint, optional)
    0,                                             -- fully_diluted_total (bigint, required)
    NULL,                                          -- option_pool_authorized (bigint, optional)
    3250645,                                       -- option_pool_outstanding (bigint, optional)
    NULL,                                          -- option_pool_available (bigint, optional)
    NULL,                                          -- option_pool_fd_percent (numeric(7,6), optional)
    NULL,                                          -- our_common_shares (bigint, optional)
    NULL,                                          -- our_preferred_shares (bigint, optional)
    4045446,                                       -- our_total_shares (bigint, optional)
    NULL,                                          -- our_ownership_percent (numeric(7,6), optional)
    0.027,                                        -- our_fd_ownership_percent (numeric(7,6), optional)
    0.1689,                                        -- our_preferred_pct (numeric(7,6), optional)
    NULL,                                          -- our_voting_pct (numeric(7,6), optional)
    0.804670264,                                   -- share_price (numeric(20,8), optional)
    27759678,                                      -- implied_valuation (numeric(20,2), optional)
    '{}',                                          -- cap_table_detail (jsonb, default '{}')
    NULL,                                          -- conversion_shares_issued (bigint, optional)
    NULL,                                          -- new_money_shares_issued (bigint, optional)
...
co_investor AS (
  INSERT INTO public.inv_co_investor (
    ...
    (SELECT id FROM new_round),                  -- financing_round_id (bigint, required, FK -> inv_financing_round)
    NULL,                                        -- external_id (text, optional)
    'BSV Fund XI, LLC',                          -- investor_name (text, required, unique per round)
    NULL,                                        -- investor_type (text, optional)
    'lead',                                      -- relationship (text, default 'participant': lead|co_lead|participant|follow_on)
    false,                                       -- has_board_seat (boolean, default false)
    true,                                        -- is_major_investor (boolean, default false)
    NULL,                                        -- amount_invested (numeric(20,2), optional)
    'USD',                                       -- currency (char(3), default 'USD')
...
INSERT INTO public.inv_information_rights (
  ...
  894826,                                      -- major_investor_threshold (numeric(20,2), optional)
  ...
  true,                                        -- year_end_balance_sheet (boolean, default false)
  true,                                        -- year_end_cap_table (boolean, default false)
  false,                                       -- year_end_income_cash_flows (boolean, default false)
  true,                                        -- year_end_stockholders_equity (boolean, default false)
...
WITH new_security AS (
  INSERT INTO public.inv_security (
    ...
    'Series B Preferred',                        -- name (text, required, unique per company)
    ...
    false,                                       -- is_valuation_reference (boolean, default false)
    '{}'                                         -- metadata (jsonb, default '{}')
  )
  RETURNING id
)
INSERT INTO public.inv_security_terms (
  ...
  NULL,                                        -- dividend_rate (numeric(8,6), optional)
...
WITH new_equity_plan AS (
  INSERT INTO public.inv_equity_plan (
    ...
    current_setting('app.company_id')::bigint,   -- company_id (bigint, required, FK -> inv_company)
    NULL,                                        -- external_id (text, optional)
    'Amended and Restated Stock Option Plan',    -- name (text, required, unique per company)
    'iso',                                       -- plan_type (text, default 'iso')
    '2026-01-13',                                -- adoption_date (date, optional)
    NULL,                                        -- expiration_date (date, optional)
    '{}'                                         -- metadata (jsonb, default '{}')
  )
  RETURNING id
...
INSERT INTO public.inv_equity_plan_snapshot (
  ...
  NULL,                                        -- external_id (text, optional)
  '2026-01-13',                                -- effective_date (date, required, unique per plan)
  3250645,                                     -- authorized_shares (bigint, optional)
  1533724,                                     -- issued_shares (bigint, optional)
  1716921,                                     -- outstanding_options (bigint, optional)
