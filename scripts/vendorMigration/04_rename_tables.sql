...
BEGIN
    ...
    -- Handle self-referencing constraint on backup vendors table
    SELECT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE table_name = backup_vendors_table 
    ...
    IF constraint_exists THEN
        EXECUTE 'ALTER TABLE public.' || quote_ident(backup_vendors_table) || 
                ' RENAME CONSTRAINT vendors_merged_into_vendor_id_fkey TO ' || 
                quote_ident(backup_vendors_table || '_merged_into_vendor_id_fkey');
        RAISE NOTICE 'Renamed vendors_merged_into_vendor_id_fkey to %_merged_into_vendor_id_fkey on backup table', backup_vendors_table;
    ELSE
        RAISE NOTICE 'vendors_merged_into_vendor_id_fkey constraint not found on backup table (this may be normal)';
    END IF;
...
-- Step 6b: Rename the self-referencing FK constraint on 'vendors.merged_into_vendor_id'
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE table_name = 'vendors' 
        AND constraint_name = 'global_vendors_merged_into_vendor_id_fkey'
        AND table_schema = 'public'
    ) THEN
        ALTER TABLE public.vendors RENAME CONSTRAINT global_vendors_merged_into_vendor_id_fkey TO vendors_merged_into_vendor_id_fkey;
        RAISE NOTICE 'Renamed global_vendors_merged_into_vendor_id_fkey to vendors_merged_into_vendor_id_fkey';
    ELSE
        RAISE NOTICE 'global_vendors_merged_into_vendor_id_fkey constraint not found - will recreate it';
        -- Recreate the self-referencing constraint if it doesn't exist
        ALTER TABLE public.vendors 
        ADD CONSTRAINT vendors_merged_into_vendor_id_fkey 
        FOREIGN KEY (merged_into_vendor_id) REFERENCES public.vendors(id) ON DELETE SET NULL;
        RAISE NOTICE 'Created vendors_merged_into_vendor_id_fkey constraint';
    END IF;
END$$;
...
-- Step 7b: Re-define the UNIQUE constraint on vendor_products_details (product_id)
ALTER TABLE public.vendor_products_details
    DROP CONSTRAINT IF EXISTS vendor_products_details_pkey;
ALTER TABLE public.vendor_products_details
    ADD CONSTRAINT vendor_products_details_pkey 
    PRIMARY KEY (product_id, contract_id, year);
...
BEGIN
    ...
    RAISE NOTICE 'All merger tracking constraints have been properly established';
END$$;
