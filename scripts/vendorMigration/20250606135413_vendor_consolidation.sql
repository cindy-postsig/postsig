...
BEGIN
    IF EXISTS (
        ...
        WHERE table_schema = 'public' AND table_name = 'vendor_products' AND column_name = 'created_at' AND is_nullable = 'NO'
    ) THEN
        RAISE NOTICE 'Altering public.vendor_products.created_at to DROP NOT NULL...';
        ALTER TABLE "public"."vendor_products" ALTER COLUMN "created_at" DROP NOT NULL;
    ELSE
        RAISE NOTICE 'Column public.vendor_products.created_at is already nullable or does not exist.';
    END IF;

    IF EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_schema = 'public' AND table_name = 'vendor_products' AND column_name = 'id'
              AND (column_default IS NULL OR column_default NOT LIKE 'nextval(''global_vendor_products_id_seq%')
    ) THEN
        RAISE NOTICE 'Altering public.vendor_products.id to SET DEFAULT nextval(''global_vendor_products_id_seq'')...';
        ALTER TABLE "public"."vendor_products" ALTER COLUMN "id" SET DEFAULT nextval('global_vendor_products_id_seq'::regclass);
    ELSE
        RAISE NOTICE 'Column public.vendor_products.id already has correct default or does not exist.';
    END IF;
END $$;
...
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_schema = 'public' AND table_name = 'vendors' AND column_name = 'created_at'
              AND (column_default IS NULL OR column_default NOT LIKE 'now(%')
    ) THEN
        RAISE NOTICE 'Altering public.vendors.created_at to SET DEFAULT now()...';
        ALTER TABLE "public"."vendors" ALTER COLUMN "created_at" SET DEFAULT now();
    ELSE
        RAISE NOTICE 'Column public.vendors.created_at already has default or does not exist.';
    END IF;

    IF EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_schema = 'public' AND table_name = 'vendors' AND column_name = 'created_at' AND udt_name <> 'timestamptz'
    ) THEN
        RAISE NOTICE 'Altering public.vendors.created_at to SET DATA TYPE timestamptz...';
    ...
    ELSE
        RAISE NOTICE 'Column public.vendors.created_at is already timestamptz or does not exist.';
    END IF;

    IF EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_schema = 'public' AND table_name = 'vendors' AND column_name = 'email' AND data_type <> 'text'
    ) THEN
        RAISE NOTICE 'Altering public.vendors.email to SET DATA TYPE text...';
        ALTER TABLE "public"."vendors" ALTER COLUMN "email" SET DATA TYPE text USING "email"::text;
    ELSE
        RAISE NOTICE 'Column public.vendors.email is already text or does not exist.';
    END IF;

    IF EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_schema = 'public' AND table_name = 'vendors' AND column_name = 'id'
              AND (column_default IS NULL OR column_default NOT LIKE 'nextval(''global_vendors_id_seq%')
    ) THEN
        RAISE NOTICE 'Altering public.vendors.id to SET DEFAULT nextval(''global_vendors_id_seq'')...';
        ALTER TABLE "public"."vendors" ALTER COLUMN "id" SET DEFAULT nextval('global_vendors_id_seq'::regclass);
    ELSE
        RAISE NOTICE 'Column public.vendors.id already has correct default or does not exist.';
    END IF;
END $$;
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE schemaname = 'public' AND tablename = 'organization_vendor_settings' AND policyname = 'Authenticated users can view org vendor settings'
    ) THEN
        RAISE NOTICE 'Creating policy "Authenticated users can view org vendor settings"...';
        CREATE POLICY "Authenticated users can view org vendor settings"
        ON "public"."organization_vendor_settings" AS permissive FOR SELECT TO authenticated USING (true);
    ELSE
        RAISE NOTICE 'Policy "Authenticated users can view org vendor settings" already exists.';
    END IF;

    IF NOT EXISTS (
        SELECT 1 FROM pg_policies
        WHERE schemaname = 'public' AND tablename = 'vendor_products' AND policyname = 'Authenticated users can view global_vendor_products'
    ) THEN
        RAISE NOTICE 'Creating policy "Authenticated users can view global_vendor_products"...';
        CREATE POLICY "Authenticated users can view global_vendor_products"
        ON "public"."vendor_products" AS permissive FOR SELECT TO authenticated USING (true);
    ELSE
        RAISE NOTICE 'Policy "Authenticated users can view global_vendor_products" already exists.';
    END IF;

    IF NOT EXISTS (
        SELECT 1 FROM pg_policies
        WHERE schemaname = 'public' AND tablename = 'vendors' AND policyname = 'Authenticated users can view global_vendors'
    ) THEN
        RAISE NOTICE 'Creating policy "Authenticated users can view global_vendors"...';
        CREATE POLICY "Authenticated users can view global_vendors"
        ON "public"."vendors" AS permissive FOR SELECT TO authenticated USING (true);
    ELSE
        RAISE NOTICE 'Policy "Authenticated users can view global_vendors" already exists.';
    END IF;
END $$;
